...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      dists[i, j] = (x_train[i] - x_test[j]).view(-1).square().sum()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  v1 = x_train.view(x_train.shape[0], -1)
  v2 = x_test.view(x_test.shape[0], -1)
  for i in range(num_train):
    dists[i] = (v1[i] - v2).square().sum(dim=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  v = x_train.view(x_train.shape[0], -1) # (M, D)
  w = x_test.view(x_test.shape[0], -1) # (N, D)
  v2 = v.square().sum(dim=1, keepdim=True) # (M, 1)
  w2 = w.square().sum(dim=1, keepdim=True) # (N, 1)
  vw = torch.matmul(v, w.T) # (M, N)
  dists = v2 + w2.T - 2*vw
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _, indices = dists.topk(dim=0, k=k, largest=False)
  y_pred, _ = y_train[indices].mode(dim=0) 
  '''
  在官方文档中找不到torch.mode()如何处理存在多个众数的情况，
  经过一定的测试，torch.mode()会返回最小的一个，符合题意
  猜测其实现中使用了排序。
  '''
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      knnClassifier = KnnClassifier(
        torch.cat(x_train_folds[:i] + x_train_folds[i+1:], dim=0),
        torch.cat(y_train_folds[:i] + y_train_folds[i+1:], dim=0),
      )
      k_to_accuracies[k].append(
        knnClassifier.check_accuracy(
          x_train_folds[i],
          y_train_folds[i],
          k=k
        )
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  mean_accs = {k: sum(v)/len(v) for k, v in k_to_accuracies.items()}
  max_acc = max(mean_accs.values())
  for k, acc in mean_accs.items():
    if acc == max_acc:
      best_k = k
      break
  ##############################################################################
